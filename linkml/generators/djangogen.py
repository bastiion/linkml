...
from linkml.generators import PYTHON_GEN_VERSION, DJANGO_GEN_VERSION
...
class MethodSignature:
    ...
    def __init__(self, methodName: str, arguments: Dict[str, str]):
...
class PythonGenerator(Generator):
    ...
    generatorversion = DJANGO_GEN_VERSION
    valid_formats = ["py"]
...
"""
    ...
    def gen_typedefs(self) -> str:
        """Generate django model declarations for all defined types"""
        rval = []
        defs_to_generate = [
    ...
    def gen_classdef(self, cls: ClassDefinition) -> str:
        ...
        parentref = f'({self.formatted_element_name(cls.is_a, True) if cls.is_a else "Model"})'
    ...
    def gen_class_variable(
        ...
        return f"""{slotname} = models.{slot_range}"""
    ...
    def gen_django_model_function(self, range: str) -> Optional[str]:
        """
        Generate a Django model function for the supplied range list

        :param rangelist: list of ranges
        :return: Django model function
        """
        switch = {
            "str": "TextField",
            "int": "IntegerField",
        ...
        fun = switch.get(range)
        return fun

    def django_reference_type(
            self, slot: SlotDefinition, cls: Optional[ClassDefinition]
    ) -> MethodSignature:
        """
        Retrun django model and parameters based on slot reference
        :param slot:
        :param cls:
        :return: tuple of call signature and parameters
        """
        rangelist = (
        ...
        primary_type = rangelist[0]
        advanced_type = rangelist[1] if len(rangelist) > 1 else None

        is_primitive = slot.range in self.schema.enums or slot.range in self.schema.types

        pkey = self.class_identifier(slot.range)
        options: Dict = {}
        # Special case, inlined, identified range
        if slot.identifier:
            options["primary_key"] = wrapInQuotes(pkey)
        ...
        if slot.multivalued:
            ...
            if is_primitive:
                ...
                if advanced_type:
                    if advanced_type == "XSDDate":
                        method_name = "DateField"
                    elif advanced_type == "XSDTime":
                        method_name = "TimeField"
                    elif advanced_type == "XSDDateTime":
                        method_name = "DateTimeField"
                call_sig = MethodSignature(method_name, {**{
    ...
    def class_reference_type(
        ...
        # if slot.annotations and hasattr(slot.annotations, "django_additional_attributes"):
        #    options += slot.annotations.django_additional_attributes
        method_sig = self.django_reference_type(slot, cls)
        return str(method_sig), prox_type, prox_type_name
